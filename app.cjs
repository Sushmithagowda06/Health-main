...
(async () => {
  try {
    const { rows } = await pool.query(
      `SELECT phone_number, patient_name, date, time, time_value FROM appointments`
    );
...
async function saveAppointment(record, doctor) {
  const { rows } = await pool.query(
    `INSERT INTO appointments
     (phone_number, patient_name, date, time, time_value, address, location_link, doctor_name, doctor_specialization)
...
app.get("/api/admin/appointments", async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT id, patient_name, phone_number, date, time,
